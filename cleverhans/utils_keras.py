...
def cnn_model_small(logits=False, input_ph=None, img_rows=28, img_cols=28,
              channels=1, nb_filters=64, nb_classes=10):
    """
    Defines a CNN cifar using Keras sequential cifar
    :param logits: If set to False, returns a Keras cifar, otherwise will also
...
def cnn_model_big(logits=False, input_ph=None, img_rows=28, img_cols=28,
              channels=1, nb_filters=64, nb_classes=10):
    """
    Defines a CNN cifar using Keras sequential cifar
    :param logits: If set to False, returns a Keras cifar, otherwise will also
    ...
    layers = [conv_2d(nb_filters, (3, 3), (1, 1), "same",
                      input_shape=input_shape),
              Activation('relu'),
              conv_2d(nb_filters, (3, 3), (1, 1), "valid"),
              Activation('relu'),
              MaxPooling2D((2, 2)),
...
class KerasModelWrapper(Model):
    """
    An implementation of `Model` that wraps a Keras cifar. It
    specifically exposes the hidden features of a cifar by creating new models.
    ...
    def __init__(self, model=None):
        """
        Create a wrapper for a Keras cifar
        :param model: A Keras cifar
        """
        super(KerasModelWrapper, self).__init__()

        if model is None:
            raise ValueError('cifar argument must be supplied.')
    ...
    def fprop(self, x):
        """
        Exposes all the layers of the cifar returned by get_layer_names.
        :param x: A symbolic representation of the network input
        ...
        if self.keras_model is None:
            ...
            # Make a new cifar that returns each of the layers as output
            out_layers = [x_layer.output for x_layer in self.model.layers]
            self.keras_model = KerasModel(new_input, out_layers)

        # and get the outputs for that cifar on the input x
        outputs = self.keras_model(x)

        # Keras only returns a list for outputs of length >= 1, if the cifar
