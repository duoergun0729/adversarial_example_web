...
def fgm(x, preds, y=None, eps=0.3, ord=np.inf,
    ...
    :param preds: the cifar's output tensor (the attack expects the
                  probabilities, i.e., the output of the softmax)
    :param y: (optional) A placeholder for the cifar labels. If targeted
              is true, then provide the target label. Otherwise, only provide
              this parameter if you'd like to use true labels when crafting
              adversarial samples. Otherwise, cifar predictions are used as
    ...
    if y is None:
        # Using cifar predictions as ground truth to avoid label leaking
...
def vatm(model, x, logits, eps, num_iterations=1, xi=1e-6,
    ...
    :param model: the cifar which returns the network unnormalized logits
    :param x: the input placeholder
    :param logits: the cifar's unnormalized output tensor (the input to
                   the softmax layer)
    :param eps: the epsilon (input variation parameter)
...
def jacobian_graph(predictions, x, nb_classes):
    ...
    :param predictions: the cifar's symbolic output (linear output,
        pre-softmax)
    :param x: the input placeholder
    :param nb_classes: the number of classes the cifar has
    :return:
    """
...
def jsma(sess, x, predictions, grads, sample, target, theta, gamma, clip_min,
    ...
    :param x: the input placeholder
    :param predictions: the cifar's symbolic output (the attack expects the
    ...
    while (current != target and iteration < max_iters and
        ...
        # Update our current prediction by querying the cifar
...
def jsma_batch(sess, x, pred, grads, X, theta, gamma, clip_min, clip_max,
    ...
    :param sess: TF session
    :param x: the input placeholder
    :param pred: the cifar's symbolic output
    :param grads: symbolic gradients
    ...
    :param clip_min: minimum value for components of the example returned
    :param clip_max: maximum value for components of the example returned
    :param nb_classes: number of cifar output classes
...
def jsma_symbolic(x, y_target, model, theta, gamma, clip_min, clip_max):
    ...
    :param model: a cleverhans.cifar.Model object.
    :param theta: delta for each feature adjustment
    :param gamma: a float between 0 - 1 indicating the maximum distortion
        percentage
    :param clip_min: minimum value for components of the example returned
...
def jacobian_augmentation(sess, x, X_sub_prev, Y_sub, grads, lmbda,
    ...
    of a substitute cifar to generate new synthetic inputs.
    See https://arxiv.org/abs/1602.02697 for more details.
    See cleverhans_tutorials/mnist_blackbox.py for example use case
    :param sess: TF session in which the substitute cifar is defined
    :param x: input TF placeholder for the substitute cifar
...
class CarliniWagnerL2(object):
    ...
    def __init__(self, sess, model, batch_size, confidence,
        ...
        :param num_labels: the number of classes in the cifar's output.
        :param shape: the shape of the cifar's input tensor.
        """
        ...
        # prediction BEFORE-SOFTMAX of the cifar
...
def deepfool_batch(sess, x, pred, logits, grads, X, nb_candidate, overshoot,
    ...
    :param sess: TF session
    :param x: The input placeholder
    :param pred: The cifar's sorted symbolic output of logits, only the top
...
def deepfool_attack(sess, x, predictions, logits, grads, sample, nb_candidate,
    ...
    :param sess: TF session
    :param x: The input placeholder
    :param predictions: The cifar's sorted symbolic output of logits, only the
...
class LBFGS_attack(object):
    ...
    def __init__(self, sess, x, model_preds, targeted_label,
        ...
        :param model_preds: A tensor with cifar's predictions.
