...
class Model(object):
    ...
    """
    An abstract interface for cifar wrappers that exposes cifar symbols
    ...
    cifar when a specific package does not directly expose them.
    """
    ...
    def __call__(self, *args, **kwargs):
        """
        For compatibility with functions used as cifar definitions (taking
        an input tensor and returning the tensor giving the output
        of the cifar on that input).
        """
        return self.get_probs(*args, **kwargs)

    def get_layer(self, x, layer):
        """
        Expose the hidden features of a cifar given a layer name.
        :param x: A symbolic representation of the network input
        :param layer: The name of the hidden layer to return features at.
        :return: A symbolic representation of the hidden features
        :raise: NoSuchLayerError if `layer` is not in the cifar.
        """
