...
class Attack(object):
    ...
    def __init__(self, model, back='tf', sess=None):
        """
        :param model: An instance of the cleverhans.cifar.Model class.
        :param back: The backend_mnist to use. Currently 'tf' is the only option.
        ...
        if not isinstance(model, Model):
            raise ValueError("The cifar argument should be an instance of"
                             " the cleverhans.cifar.Model class.")
    ...
    def generate(self, x, **kwargs):
        ...
        :param x: The cifar's symbolic inputs.
    ...
    def get_or_guess_labels(self, x, kwargs):
        ...
        use that as the label.
        Otherwise, use the cifar's prediction as the label and perform an
        untargeted attack.
        """
        import tensorflow as tf
...
class FastGradientMethod(Attack):
    ...
    def __init__(self, model, back='tf', sess=None):
        ...
        Note: the cifar parameter should be an instance of the
        cleverhans.cifar.Model abstraction provided by CleverHans.
        """
        if not isinstance(model, Model):
    ...
    def generate(self, x, **kwargs):
        ...
        :param y: (optional) A tensor with the cifar labels. Only provide
...
class BasicIterativeMethod(Attack):
    ...
    def generate(self, x, **kwargs):
        ...
        :param y: (optional) A tensor with the cifar labels.
        :param y_target: (optional) A tensor with the labels to target. Leave
                         y_target=None if y is also set. Labels should be
                         one-hot-encoded.
        :param ord: (optional) Order of the norm (mimics Numpy).
        ...
        # Fix labels to the first cifar predictions for loss computation
...
class StochasticMomentumIterativeMethod(Attack):
    ...
    def __init__(self, model, back='tf', sess=None):
        ...
        super(StochasticMomentumIterativeMethod, self).__init__(model, back, sess)
        self.feedable_kwargs = {'eps': np.float32,
                                'eps_iter': np.float32,
                                'y': np.float32,
                                'y_target': np.float32,
...
class SaliencyMapMethod(Attack):
    ...
    def parse_params(self, theta=1., gamma=1., nb_classes=None,
        ...
        :param theta: (optional float) Perturbation introduced to modified
                      components (can be positive or negative)
        :param gamma: (optional float) Maximum percentage of perturbed features
        :param nb_classes: (optional int) Number of cifar output classes
        :param clip_min: (optional float) Minimum component value for clipping
        :param clip_max: (optional float) Maximum component value for clipping
        :param y_target: (optional) Target tensor if the attack is targeted
        """
...
class DeepFool(Attack):
    ...
    def generate(self, x, **kwargs):
        ...
        :param nb_classes: The number of cifar output classes
...
class FastFeatureAdversaries(Attack):
    ...
    against a target internal representation of a cifar.
    ...
    def attack_single_step(self, x, eta, g_feat):
        ...
        :param x: the input placeholder
        :param eta: A tensor the same shape as x that holds the perturbation.
        :param g_feat: cifar's internal tensor for guide
        :return: a tensor for the adversarial example
        """
        import tensorflow as tf
        from cleverhans.utils_tf import clip_eta
